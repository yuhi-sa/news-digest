...
_STAGE1_PROMPT = """\
あなたは最新研究の要点を実務エンジニアに伝えるテックライターです。
...
### 📖 この研究が解こうとしている問題
この論文が取り組む**具体的な問題**にフォーカスする。5〜8文。
抽象的な理論の説明ではなく、「どんなシステムで何が困っているのか」を描写すること。
既存手法の限界と、なぜ新しいアプローチが必要なのかを明確にする。
...
### 🌍 実務への応用可能性
この研究の成果が実務のソフトウェア開発やシステム運用にどう活かせるか。5〜8文。
- どのような場面で使えそうか
- 既存の技術・ツール・フレームワークとの関係
- 読者が自分のプロジェクトに取り入れるとしたら何から始めるか
...
def summarize_paper(paper: Paper, api_key: str | None) -> str:
    ...
    Stage 2: Architecture diagram + contributions + applications + keywords
    ...
    fmt_args = {
        "title": paper.title,
        "authors": authors_str,
        "published": paper.published or "不明",
    ...
    # Stage 2
    logger.info("Stage 2: generating diagram, contributions, and applications")
    stage2 = call_gemini(
